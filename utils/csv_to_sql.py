    ...
    with open('sample_data/client.csv') as csvfile:
        ...
        for row in reader:
            ...
            csv2db_client = {
                ...
                "gender": cast_int(row["Gender"]),
                "veteran": cast_int(row["VeteranStatus"]),
                "year_entered": cast_int(row["YearEnteredService"]),
                "year_exited": cast_int(row["YearSeparated"]),
                "military_branch": cast_int(row["MilitaryBranch"]),
        ...
        Client.objects.bulk_create(bulk_create)
...
def sync_disabilities():
    with open('sample_data/disabilities.csv') as csvfile:
        ...
        for row in reader:
            ...
            try:
                csv2db_client = {
                    ...
                    "disability_response":cast_int(row['DisabilityResponse']),
        ...
        Disabilities.objects.all().delete()
        Disabilities.objects.bulk_create(bulk_create)
...
def sync_employmenteducation():
    with open('sample_data/employmenteducation.csv') as csvfile:
        ...
        EmploymentEducation.objects.all().delete()
        EmploymentEducation.objects.bulk_create(bulk_create)
...
def sync_enrollment():
    with open('sample_data/enrollment.csv') as csvfile:
        ...
        Enrollment.objects.bulk_create(bulk_create)
...
def sync_exit():
    with open('sample_data/exit.csv') as csvfile:
        ...
        Exit.objects.bulk_create(bulk_create)
    ...
    with open('sample_data/healthanddv.csv') as csvfile:
        ...
        for row in reader:
            ...
            csv2db_client = {
                ...
                "data_collection_stage":row["DataCollectionStage"],
                "date_created":fmt_datetime(row["DateCreated"]),
                "date_updated":fmt_datetime(row["DateUpdated"]),
                "associate_id":row["UserID"]
            }
        ...
        HealthAndDV.objects.all().delete()
        HealthAndDV.objects.bulk_create(bulk_create)

def sync_income_benefits():
    with open('sample_data/incomebenefits.csv') as csvfile:
        reader = csv.DictReader(csvfile)
        bulk_create = []
        ...
        fmt_datetime = lambda k: parse(k) if k else datetime.datetime.now()
        cast_int = lambda k: int(k) if k else None
        cast_float = lambda k: float(k) if k else None
        cast_bool = lambda k : bool(k) if k else False
        for row in reader:
            for key in row:
                if row[key] == 'NULL':
                    row[key] = None
            ...
            csv2db_client = {
                ...
                "personal_id":row["PersonalID"],
                "information_date":fmt_date(row["InformationDate"]),
                "income_from_any_source":cast_float(row["IncomeFromAnySource"]),
                "total_monthly_income":cast_float(row["TotalMonthlyIncome"]),
                "earned":cast_bool(row["Earned"]),
                "earned_amount":cast_float(row["EarnedAmount"]),
                "unemployment":cast_bool(row["Unemployment"]),
                "unemployment_amount":cast_float(row["UnemploymentAmount"]),
                "ssi":cast_bool(row["SSI"]),
                "ssi_amount":cast_float(row["SSIAmount"]),
                "ssdi":cast_bool(row["SSDI"]),
                "ssdi_amount":cast_float(row["SSDIAmount"]),
                "va_disability_service":cast_bool(row["VADisabilityService"]),
                "va_disability_service_amount":cast_float(row["VADisabilityServiceAmount"]),
                "va_disability_non_service":cast_bool(row["VADisabilityNonService"]),
                "va_disability_non_service_amount":cast_float(row["VADisabilityNonServiceAmount"]),
                "private_disability":cast_bool(row["PrivateDisability"]),
                "private_disability_amount":cast_float(row["PrivateDisabilityAmount"]),
                "workers_comp":cast_bool(row["WorkersComp"]),
                "workers_comp_amount":cast_float(row["WorkersCompAmount"]),
                "tanf":cast_bool(row["TANF"]),
                "tanf_amount":cast_float(row["TANFAmount"]),
                "ga":cast_bool(row["GA"]),
                "ga_amount":cast_float(row["GAAmount"]),
                "soc_sec_retirement":cast_bool(row["SocSecRetirement"]),
                "soc_sec_retirement_amount":cast_float(row["SocSecRetirementAmount"]),
                "pension":cast_bool(row["Pension"]),
                "pension_amount":cast_float(row["PensionAmount"]),
                "child_support":cast_bool(row["ChildSupport"]),
                "child_support_amount":cast_float(row["ChildSupportAmount"]),
                "alimony":cast_bool(row["Alimony"]),
                "alimony_amount":cast_float(row["AlimonyAmount"]),
                "other_income_source":cast_bool(row["OtherIncomeSource"]),
                "other_income_source_amount":cast_float(row["OtherIncomeAmount"]),
                ...
                "hiv_aids_assistance":cast_bool(row["HIVAIDSAssistance"]) or False,
                "no_hiv_aids_assistance_reason":row["NoHIVAIDSAssistanceReason"],
                "adap":cast_bool(row["ADAP"]) or False,
        ...
        IncomeBenefits.objects.all().delete()
        IncomeBenefits.objects.bulk_create(bulk_create)
    ...
    with open('sample_data/services.csv') as csvfile:
        ...
        for row in reader:
            ...
            csv2db_client = {
                ...
                "sub_type_provided" : cast_float(row["SubTypeProvided"]),
                "fa_amount" : cast_float(row["FAAmount"]),
                "date_created" : fmt_datetime(row["DateCreated"]),
                "date_updated" : fmt_datetime(row["DateUpdated"]),
                "associate_id" : row["UserID"]
            }
        ...
        Services.objects.bulk_create(bulk_create)
